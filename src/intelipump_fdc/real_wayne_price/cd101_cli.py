"""CLI: intelipump-real-wayne-cd101-request — single-shot CD101 totals request."""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="intelipump-real-wayne-cd101-request",
        description=(
            "Technician-supervised real-Wayne CD101 request totals (read-only). "
            "Transmits exactly one CD101 DATA frame (counter-select default 1, "
            "matching ePump pre-OUT captures). Does not send RESET or AUTHORIZE."
        ),
    )
    ...
    p.add_argument("--ack-timeout-ms", type=int, default=1000)
    p.add_argument("--post-write-settle-ms", type=int, default=200)
    p.add_argument("--post-write-max-status-polls", type=int, default=8)
    p.add_argument("--sequence", type=int, default=0)
    p.add_argument("--counter-select", type=int, default=1)
    ...
    p.add_argument("--confirm-execute-cd101-request", action="store_true")
    p.add_argument(
...
def _params_from_args(args: argparse.Namespace) -> Cd101WriteParams:
    confirms = Cd101WriteConfirmations(
    ...
    return Cd101WriteParams(
        ...
        counter_select=args.counter_select,
...
async def _async_main(argv: list[str] | None) -> int:
    ...
    try:
        ...
        validate_cd101_write_settings(settings)
    except PollBenchRefusedError as exc:
        print(f"CD101_REQUEST_REFUSED: {exc}", file=sys.stderr)
        return 2
    ...
    session = Cd101WriteSession(transport, params, canonical_port=canonical)
    try:
    ...
    if args.json:
        ...
        print(f"dc101Observed: {result.summary.get('dc101Observed')}")
