...
sed -i \
  ...
  "$TMP_ENV"
# Lock SET_PRICE ownership (critical when cloning a PMS Pi onto AGO).
if ! grep -q '^INTELIPUMP_LOGICAL_PUMP_ID=' "$TMP_ENV"; then
  printf '\nINTELIPUMP_LOGICAL_PUMP_ID=%s\n' "$PUMP_CODE" >>"$TMP_ENV"
else
  sed -i "s|^INTELIPUMP_LOGICAL_PUMP_ID=.*|INTELIPUMP_LOGICAL_PUMP_ID=${PUMP_CODE}|" "$TMP_ENV"
fi
if ! grep -q '^INTELIPUMP_PRODUCT=' "$TMP_ENV"; then
  printf 'INTELIPUMP_PRODUCT=%s\n' "$PRODUCT" >>"$TMP_ENV"
else
  sed -i "s|^INTELIPUMP_PRODUCT=.*|INTELIPUMP_PRODUCT=${PRODUCT}|" "$TMP_ENV"
fi
...
if [[ "$INSTALL_SYNC" -eq 1 ]]; then
  ...
  sed -i \
    ...
    "$TMP_SYNC_ENV"
  if ! grep -q '^INTELIPUMP_LOGICAL_PUMP_ID=' "$TMP_SYNC_ENV"; then
    printf '\nINTELIPUMP_LOGICAL_PUMP_ID=%s\n' "$PUMP_CODE" >>"$TMP_SYNC_ENV"
  else
    sed -i "s|^INTELIPUMP_LOGICAL_PUMP_ID=.*|INTELIPUMP_LOGICAL_PUMP_ID=${PUMP_CODE}|" "$TMP_SYNC_ENV"
  fi
  if ! grep -q '^INTELIPUMP_PRODUCT=' "$TMP_SYNC_ENV"; then
    printf 'INTELIPUMP_PRODUCT=%s\n' "$PRODUCT" >>"$TMP_SYNC_ENV"
  else
    sed -i "s|^INTELIPUMP_PRODUCT=.*|INTELIPUMP_PRODUCT=${PRODUCT}|" "$TMP_SYNC_ENV"
  fi
